...
### 认不出的调用要**安静**,不要报错(线上打回来的教训)

第一版对没鉴权、没会话头、不认识的 agent、坏 JSON 一律回 4xx。上线后每个 Claude 会话
每一轮都会往用户脸上打一行:
...
原因是 **agent 把 hook 的非 2xx 当成错误显示给用户**,而「不是托管会话打来的」恰恰是
**完全正常**的事 —— hook 装在用户级全局配置里,这台机器上每一个 Claude 会话都会触发它,
其中绝大多数不是从 octoterm 里起的。把设计上必然发生的事报成错误,等于让用户天天看红字。

设计时我把这条只当成「服务端日志噪音」记下了,漏了它会**反向显示在 agent 那一侧**。

现在:除「非回环」外,所有认不出的调用一律 **200 + 空体**。对 agent 的语义恰好是
「收到了,没有决定」,它照常走自己的流程。安全性由**什么都不做**保证,而不是由回一个
错误码保证 —— 测试断言的也从「返回 401」改成了更强的「200 且会话表没被写进去」。

诊断没丢:服务端按 60 秒节流记一条 debug 日志,`GET /api/agents` 的自检照样能报出
`StalePort`。**已知缺口**:装了但 token 从来对不上(例如 server 重启过而会话是老的)
目前只表现为「列表一直是空的」,没有专门的自检项。
