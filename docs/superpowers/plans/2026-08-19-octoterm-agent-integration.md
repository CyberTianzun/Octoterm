...
## 实施记录:Task 7(2026-08-19,已完成)

清理判决 `decide()` 是纯函数:`now` 与「托管会话是否还活着」都由调用方给,不读时钟、
不查进程,于是 9 条规则可以逐条断言而不引入任何时间相关的 flaky。

规则顺序:**托管会话没了 ⇒ 立即删**(pty 子进程退出意味着里面的 agent 必死,比任何超时
都硬)> **正在等人的一律不扫**(人可能在睡觉,挂起请求自己有超时)> 超 `session_stale`
删 > 超 `working_stale` 且状态是 working/thinking 转 idle。空闲基准取
`max(updated_at, acked_at)`。

`MarkIdle` **不刷新时间戳**,`mark_idle_does_not_extend_life` 盯着这条:参考实现在这里
踩过坑,刷新之后「超时转 idle」会不断把删除时刻往后推,那条会话永远删不掉。

**被测试抓到的一个真问题**:`tokio::time::interval` 的第一跳是**立即**触发的,而扫描任务
的首次轮询会被推迟到运行时下一次调度 —— 于是那个「立即」可能落在启动之后的任意一点,
把刚建好的会话当场扫掉(测试里没有真实 pty 会话,`host_alive` 恒为 false)。改用
`interval_at` 把第一跳推到一个周期之后,启动瞬间不做任何判断。

**扫描器只会真正启动一次**:desktop 重建 HTTP 层时 `serve()` 会再调一次 `start_sweeper`,
用 `AtomicBool::swap` 挡住,不能因此多出一个扫描器。任务持 `Weak`,store 没人要了就退出。

**一条要记的教训(关于我自己的验证方式)**:之前几次我用
`grep '^test result' | awk '{p+=$4; f+=$6}'` 汇总,而 `FAILED` 行的字段布局与 `ok` 行不同,
于是**失败被算成了 0**。现在改成显式数 `^test result: FAILED` 的行数。承诺「全绿」的地方,
判据本身必须先是对的。
...
## 实施记录:Task 8(2026-08-19,已完成 3.5/4 步)

web 端 66 个用例通过、`tsc --noEmit` 干净、`npm run build` 正常;Rust 侧 248 用例、
0 失败、clippy 干净。

**Step 2 的「自由文本回答」是按另一种形态做的,这是有意的**:没有在横幅里再放一个
输入框,而是给一个「去这个会话」按钮,一键 attach 过去,直接在终端里打字。

理由是这个功能的立足点本身:octoterm 托管着那个 pty,**终端就是那个自由文本输入框**。
再造一个只会多一条容易出错的路径,而且它处理不了 agent 自己画的那些 TUI 交互
(方向键选菜单、多行编辑)。结构化的允许/拒绝放横幅,自由文本回归终端 —— 各归其位。

**Step 3 已补齐**(2026-08-19 稍后):diff 预演做完了。「看看会改什么」和 install 走的是
**同一份计划**,只是不落盘 —— 「先看后装」不是另一条代码路径,而是同一条路径的干跑,
所以看到的就是将要发生的。预演只列「哪个文件、涉及哪些事件」,不展开 hook 的完整
JSON:那是给机器看的,一屏塞不下,而用户真正要判断的是「你要动我哪个文件、动几处」。
冲突导致只装遥测时,预演里会先亮一行说明。

**被现有测试挡下的一个坏写法**:一开始用 ``t(`agent.state.${s.state}`)`` 拼词条键。
仓库里有一条「没有没人引用的死词条」的测试,它扫不到模板字面量拼出来的键 —— 而这
恰恰说明这种写法同时绕过了两道防线:`MsgKey` 的类型检查(只能写 `as never`)和那条
死词条测试。改成显式的 `Record<AgentState, MsgKey>` 映射,两道防线都回来了。

同一条测试还揪出我加了 `agent.answered` 却没用上。删掉,而不是硬造一个用途。

**`SettingsHost` 多了 `token()`**:AI 集成页要打 `/api/`,而设置模块此前只拿配置。
