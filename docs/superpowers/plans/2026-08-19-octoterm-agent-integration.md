...
- [~] **Step 3: 设置页**(装/卸与冲突提示已做,**diff 预演未做**)
...
**Step 3 只做了一半,如实记账**:agent 清单、检测依据、集成状态(含 `stale-port`)、
冲突提示、装/卸按钮都做了;**`GET /api/agents/{id}/plan` 的 diff 预演没做**。
端点是现成的,缺的是 UI。装 hook 改的是用户的文件,「先看后装」不该长期缺席,
但它不阻塞这一期能用。
